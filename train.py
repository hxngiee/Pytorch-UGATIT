...
class Train:
    def __init__(self, args):
        ...
        self.gpu_mode = 'multi' if self.gpu_devices else "single"

    def save(self, dir_chck, netG_a2b, netG_b2a, netD_a, netD_b, optimG, optimD, epoch, gpu_mode):
        ...
        if gpu_mode =='single':
            ...
            torch.save({'netG_a2b': netG_a2b.module.state_dict(), 'netG_b2a': netG_b2a.module.state_dict(),
                       'netD_a': netD_a.module.state_dict(), 'netD_b' : netD_b.module.state_dict(),
                       'optimG': optimG.state_dict(), 'optimD': optimD.state_dict()},
                      "%s/model_epoch%04d.pth.tar" % (dir_chck, epoch), _use_new_zipfile_serialization=False)

    def load(self, dir_chck, netG_a2b, netG_b2a, netD_a=[], netD_b=[], optimG=[], optimD=[], epoch=[], mode='train',gpu_mode='single'):
        if not epoch:
        ...
        dict_net = torch.load('%s/model_epoch%04d.pth.tar' % (dir_chck, epoch), map_location=self.device)
        ...
        if mode == 'train':
            if gpu_mode == 'single':
                ...
                netG_a2b.module.load_state_dict(dict_net['netG_a2b'])
                netG_b2a.module.load_state_dict(dict_net['netG_b2a'])
                netD_a.module.load_state_dict(dict_net['netD_a'])
                netD_b.module.load_state_dict(dict_net['netD_b'])
    ...
    def train(self, gpu, ngpus_per_node, args):
        ...
        transform_train = transforms.Compose(
            [Normalize(), RandomFlip(), Rescale((self.ny_load, self.nx_load)), RandomCrop((self.ny_in, self.nx_in)),
             ToTensor()])
        transform_inv = transforms.Compose([ToNumpy(), Denomalize()])

        dataset_train = Dataset(dir_data_train, direction=self.direction, data_type=self.data_type,
        ...
        netD_a_param = sum(p.numel() for p in netD_a.parameters() if p.requires_grad)
        netD_b_param = sum(p.numel() for p in netD_b.parameters() if p.requires_grad)

        print('The number of netG parameters of model is', netG_a2b_param)
        print('The number of netG parameters of model is', netG_b2a_param)
        print('The number of netG parameters of model is', netD_a_param)
        print('The number of netG parameters of model is', netD_b_param)
        ...
        if train_continue == 'on':
            netG_a2b, netG_b2a, netD_a, netD_b, optimG, optimD, st_epoch = \
                self.load(dir_chck, netG_a2b, netG_b2a, netD_a, netD_b, optimG, optimD, mode=mode, gpu_mode=gpu_mode)
        ...
        for epoch in range(st_epoch + 1, num_epoch + 1):
            ...
            if args.rank == 0:
                if (epoch % num_freq_save) == 0:
                    self.save(dir_chck, netG_a2b, netG_b2a, netD_a, netD_b, optimG, optimD, epoch, gpu_mode)
    ...
    def test(self):
        ...
        init_net(netG_a2b, init_type='normal', init_gain=0.02, gpu_ids=gpu_id)
        init_net(netG_b2a, init_type='normal', init_gain=0.02, gpu_ids=gpu_id)
